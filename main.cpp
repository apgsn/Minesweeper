...
int main(int argc, char* argv[]){
  ...
  if(argc != 5 || (atoi(argv[1]) * atoi(argv[2]) - 9) < atoi(argv[4])){
    std::cout << "Could not load valid args, using config.dat values instead.\n";
    if(!std::freopen("config.dat", "r", stdin)){
      std::cout << "Could not load config.dat. Program terminated.\n";
      return 0;
    }
    std::string d;
    std::cin >> d >> width >> d >> height >> d >> size >> d >> mines;
  } else {
    width = atoi(argv[1]);
    height = atoi(argv[2]);
    size = atoi(argv[3]);
